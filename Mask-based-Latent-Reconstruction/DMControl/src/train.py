    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                episode_step += 1
                if episode_step == args.horizon_step:
                    done = True
...
def main(args: DictConfig) -> None:
    ...
    args.work_dir = args.work_dir + '/' + f'{args.agent}_dmc' + '/' + args.env
    ...
    for step in tqdm(range(0, args.num_env_steps)):
        ...
        done_bool = 0 if episode_step + 1 == args.horizon_step else float(
            done)
        episode_reward += reward
        replay_buffer.add(obs, action, reward, next_obs, done_bool)
        if episode_step+1 == args.horizon_step:
            done = True
